...
[source,bash]
----
# Create all objects up to and including the job template
ansible-playbook configure_controller_staged.yml -e module=module-06

# Launch the job template separately (launches are not idempotent)
ansible-playbook configure_controller_launch.yml --tags launch-apache
----

[TIP]
====
The CaC playbooks separate **configuration** (idempotent, safe to re-run) from **launching** (not idempotent). This is a best practice for production environments.
====
