= Module 2: Introduction to Automation Controller
:notoc:
...
image::controller-dashboard.png[Automation controller overview dashboard, opts="border"]

=== Automation Controller Jobs View

You can access the **Jobs** view by clicking **Automation Execution** → **Jobs** from the left navigation bar. This view shows all the jobs that have run, including project syncs, SCM updates, playbook runs, and more.

image::jobs-view.png[Jobs view in automation controller, opts="border"]
...
image::schedules-view.png[Schedules view in automation controller, opts="border"]

---

== Lab Guide

Welcome to the *Introduction to automation controller* lab. In the following challenges, you will learn how to run your Ansible Playbooks through the automation controller's web UI — from creating inventories and importing projects to building job templates and workflows.
...
image::automation-execution-view.png[Automation Execution menu highlighted, opts="border"]
...
Ansible Automation Platform 2.7 includes a section with quick start guides and interactive how-tos, which is very useful for getting started right after an installation.

image:quick-starts.png[Quick start guides on the dashboard, opts="border"]
...
+
Click on the **View all Inventories** link from the *Overview* page or from the left navigation menu under **Automation Execution** → **Infrastructure** → **Inventories**.
...
The Demo Inventory and Demo Project you explored in this challenge come pre-loaded with the automation controller. In a CaC approach, even these default objects can be managed declaratively. Starting from the next challenge, you will see how every object you create through the UI has an equivalent YAML representation using the `infra.aap_configuration` collection.

---

== Next Steps

In the next challenge, you will create your own inventory and add hosts to manage. Press the blue **Next** button below to continue.
